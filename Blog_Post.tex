...
Our approach follows a classic machine learning pipeline with three distinct phases: **data preparation**, **model training**, and **inference serving**. Let me break down the methodology:
...
\begin{tikzpicture}[
    ...
    \node[box, below of=input] (preprocess) {Text Preprocessing};
    \node[process, below of=preprocess] (vectorize) {CountVectorizer};
    \node[box, below of=vectorize] (predict) {Naive Bayes Model};
    \node[io, below of=predict] (output) {Spam/Ham + Confidence};
    ...
    \node[box, below of=dataset] (train_split) {80/20 Train/Test Split};
    \node[process, below of=train_split] (train_vec) {Fit CountVectorizer};
    \node[box, below of=train_vec] (train_model) {Train Naive Bayes};
    \node[process, below of=train_model] (eval) {Evaluate (98.39\%)};
...
Let's start with the heart of the system: `model.py`. This file encapsulates all the ML logic into a clean, object-oriented interface. Here's why I structured it this way...
...
Now for the glue: `app.py`. This is where we expose the model to the web.
...
This is where things get interesting. I wanted to prove that \textbf{you don't always need React} to build a responsive, state-driven UI. Here's how `app.js` orchestrates everything:
...
async function analyzeMessage(message) {
    ...
    submitBtn.innerHTML = 
        `<i data-lucide="loader-2" class="btn-icon spin"></i> 
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || 
                `HTTP ${response.status}: Analysis failed`);
        }
        ...
        showError(error.message || 
            'Failed to analyze. Check console for details.');
    } finally {
        isLoading = false;
        submitBtn.disabled = false;
        submitBtn.innerHTML = 
            `<span class="btn-text">Scan Message</span> 
...
function displayResult(prediction, confidence) {
    ...
    if (isSafe) {
        ...
        resultDescription.textContent = 
            'This message appears to be legitimate and secure.';
    } else {
...
async function fetchStats() {
    try {
        ...
        // HERE'S THE FIX: Display real accuracy
...
function renderChart(hamCount, spamCount) {
    try {
        ...
        console.log(`📈 Chart: Ham=${hamCount}, Spam=${spamCount}`);
...
Looking back at the `SpamModel` class, I wanted to answer a simple question: \textbf{where does state live?} By wrapping the scikit-learn objects in a class, I get:
...
\begin{devnote}
    The UI design respects dark mode conventions: using muted purples (#7c3aed), greens (#10b981), and reds (#ef4444) against a dark background (#0a0e27). This color scheme is WCAG AA compliant and looks clean without being sterile.
\end{devnote}
...
\begin{center}
\begin{tcolorbox}[colback=white, colframe=success, title=\textbf{✓ Safe Message}]
...
\begin{center}
\begin{tcolorbox}[colback=white, colframe=danger, title=\textbf{⚠️ Spam Detected}]
